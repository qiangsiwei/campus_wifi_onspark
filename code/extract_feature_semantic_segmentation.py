...
if __name__ == "__main__":
	...
	# print " ".join([word[0] for word in Seg(p)])
	import gzip
	import fileinput
	umap, stop, wdict = {}, {}, {}
	for line in fileinput.input("stop.txt"):
		stop[line.strip()] = True
	fileinput.close()
	for line in gzip.open("_feature/semantic.txt.gz"):
		mac, segs = line.strip().split(" ")[0], [w[0] for w in Seg(line.strip().split(" ")[1])]
		if not umap.has_key(mac):
			umap[mac] = {}
		for w in segs:
			if not stop.has_key(w):
				if not wdict.has_key(w):
					wdict[w] = {}
				wdict[w][mac] = True
			umap[mac][w] = 1 if not umap[mac].has_key(w) else umap[mac][w]+1
	wlist = sorted(filter(lambda x:len(wdict[x].keys())>=100, wdict.keys()))
	print len(wlist)
	file = open("_feature/semantic_vector.txt","w")
	for mac, ws in umap.iteritems():
		file.write(mac)
		for w in wlist:
			if ws.has_key(w):
				file.write(" "+str(ws[w]))
